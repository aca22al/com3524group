...
# Value can be "P" for power plant, "I" for incinerator
# Below value is used for first time run.
fireStartAt = "I"
...
#all chaparal (0), done when creating a np.zeros array  
    ...
    if fireStartAt == "I":
        #Start fire at incinerator here:
        for i in range(0, 20):
    ...
    #  EXTEND the second forest on both sides
    for y in range(60,80):
    ...
    # Develop Forest around Waste incinerator
    for y in range(0,20):
        for x in range(170,180):
            gridray[y][x] = FOREST 
...
# NCB Check the split in time again
    ...
    #  ******   NOT REQUIRED FOR TASE 4 ***********
    
    #  Apply Intervention
    # grid =  apply_Shortterm_Intervention (grid)
...
def apply_Shortterm_Intervention(grid):

    # SHORT TERM SOLUTIONS ***************************8

    # The planning of short-term intervention in the case of a fire starting at the incinerator. 
    # Assume that there is a limited quantity of water (enough to cover only ~12.5km2) that 
    # can be dropped aerially. Use your model to explore where the best place for this drop 
    # might be.

    global applyIntervention

    # if timestep < 10:
    #     print ("1 = Time stamp =" +  str(timestep))
    #     print ("applyIntervention =" +  str(applyIntervention))


    gridWithIntervention = grid
    # print ("****Time stamp =" +  str(timestep))

    # Drop water near incinerator for TASK3:
    if (timestep == 2 and applyIntervention == True) :
    ...
    if (timestep == 12 and applyIntervention == True) :
        # Area = 20 cells (x) = 20*0.25 = 5 km = 5km^2
        for y in range(0,20):
            for x in range(180,181):
                gridWithIntervention[y][x] = LAKE 

        # Area = 20 cells (y) = 20*0.25 = 5 km = 5km^2
    ...
    # Drop water near incinerator for TASK3:
    if (timestep == 15 and applyIntervention == True) :
        # Area = 24 cells (x) = 24*0.25 = 6 km = 6km^2
        for y in range(0,24):
            for x in range(176,177):
                gridWithIntervention[y][x] = LAKE 
    ...
    if (timestep == 150 and applyIntervention == True) :
        # Drop water near TOWN  FOR TASK3:
        # Area = 20 cells (x) = 20*0.25 = 5 km = 5km^2
