...
# NCB Check the split in time again
    ...
    if (reached_town == False) & ((grid == TOWN_BLACK) & (neighbourcounts[FIRST_FLAME_YELLOW] >= 1)).any():
       ...
       print("The fire reached the town at: " + str(timestep) + " hours")
       reached_town = True
...
# will be invoked in @updateBurn() function.:
...
# probability multiplies based on burningNeighbourCount (how many neighbouring cells are on fire (yellow))
    ...
    if windDirection == "north":
        probability_With_WD = np.where((N == FIRST_FLAME_YELLOW), probability_all * 2, probability_all)
        probability_With_WD = np.where((S == FIRST_FLAME_YELLOW), probability_With_WD * 0.2, probability_With_WD)
    else:
        probability_With_WD = probability_all

    return probability_With_WD


def printResultToFile(task, timestep):
    f = open(ResultsPath + "/" + task + "results.txt", "w") 
    text = "The fire reached the town at: " + "\n" + str(timestep) + " hours"
    f.writelines(text)
    f.close()
