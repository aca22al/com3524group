...
def setup(args):
    ...
    config.title = "Task1_RelativeTime"
    config.dimensions = 2
...
#all chaparal (0), done when creating a np.zeros array  
    ...
    # NEW
    # Load task parameters - fireStartAt - Power Plant (P) or Waste Incinerator (I)
    # from file Task1_RelativeTime_Param.txt   OR
    # set the fireStartAt variable value in the begining of this code file
...
def transition_function(grid, neighbourstates, neighbourcounts):
    ...
    # same as Chaparal because in and around first fire is Chaparal, get rid of initial flames REDO 
...
def readTaskParameters(Task_Name):
    if os.path.isfile(ParametersPath + "/" + Task_Name + "_Param.txt"): 
        ...
        if params[0] != "":
            fireStartAt = params[1]

        f.close()
    else:
        f = open(ParametersPath + "/" + Task_Name + "_Param.txt", "a") 
        f.writelines("fireStartAt=P")
        f.close()
...
def printResultToFile(task, timestep):
    ...
    text = "*** Simulation Run at - " + str(current_time) + "\n" + "Wind Direction = " + windDirection + \
           "\n" + " The fire started at " + fireStartAt + \
           "\n" + " and the fire reached the town at : " + str(timestep) + " hours" + "\n"
    f.writelines(text)
    f.close()
